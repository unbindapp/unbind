      ...
      return new Promise((resolve, reject) => {
        fetchEventSource(url, {
          ...
          onopen: async (response) => {
            // fetchEventSource only treats a 2xx text/event-stream response as a
            // real connection; anything else (e.g. a 401 with a JSON body) must
            // surface as an error instead of being read as an empty stream that
            // "closes unexpectedly" and shows up as a (cancelled) request.
            const contentType = response.headers.get("content-type");
            if (!response.ok || !contentType?.startsWith("text/event-stream")) {
              const body = await response.text().catch(() => "");
              throw new Error(`SSE ${response.status} (${contentType ?? "no content-type"}): ${body}`);
            }
            if (isMountedRef.current) {
              setIsConnected(true);
              resolve(true);

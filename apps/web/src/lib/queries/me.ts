...
import type { WhoamiResponseBody } from "@/lib/server/client.gen";

export type Me = WhoamiResponseBody["data"];
...
/**
 * Session = "does GET /users/whoami return 200". We call the endpoint through the
...
export const meQuery = queryOptions({
  ...
  queryFn: async (): Promise<Me | null> => {
    const res = await apiFetch(`${getConfig().apiUrl}/users/whoami`);
    if (res.status === 401) return null;
    if (!res.ok) throw new Error("Failed to load session");
    const json = (await res.json()) as WhoamiResponseBody;
    return json.data;
  },
  staleTime: 5 * 60 * 1000,
});

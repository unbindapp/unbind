...
export default function TemplateDraftPanelContent({ templateDraft, className, ...rest }: TProps) {
  ...
  const deployMutationKey = useMemo(
    () => deployMutationKeys.template(templateDraft.id),
    [templateDraft.id],
  );
  const { isPending: isPendingDeployTemplate, error: errorDeployTemplate } =
    useSharedMutation(deployMutationKey);

  const { mutateAsync: deployTemplate } = useMutation({
    mutationKey: deployMutationKey,

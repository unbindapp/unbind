...
const DraftSchema = z.object({
  ...
  railpackBuilderInstallCommand: z.string(),
  railpackBuilderBuildCommand: z.string(),
  dockerBuilderDockerfilePath: z.string(),
  dockerBuilderBuildContext: z.string(),
...
export function UndeployedContentGit({
  ...
  const { mutateAsync: createFirstDeployment } = useMutation({
    ...
    mutationFn: async (formValues: TFormValues) => {
      ...
      if (formValues.autoDeploy !== service.config.auto_deploy) {
        extraProps.autoDeploy = formValues.autoDeploy;
      }
  ...
  const autoGeneratedDomain = useMemo(() => {
    ...
    const domain = generateDomain({ name: repo, wildcardDomain, seed: serviceId });
    return domain;
  }, [wildcardDomain, repo, serviceId]);

  const form = useAppFormWithPersistence({
    defaultValues: {
      branch: branch,
      autoDeploy: service.config.auto_deploy,
      domain: autoGeneratedDomain || "",
      isPublic: true,
      targetPort: detectedPort !== undefined ? detectedPort : "",
  ...
  const [isAdvancedSettingsOpen, setIsAdvancedSettingsOpen] = usePersistedState({
    key: `${persistenceKey}:advanced-settings`,

...
const cardVariants = cva(
  "w-full select-text group/card rounded-xl flex flex-col items-center p-1.5 gap-1 text-center text-destructive",
  {
    variants: {
      variant: {
        default: "border border-destructive/5-10 bg-destructive/1-10",
        "no-bg": "",
      },
    },
    defaultVariants: {
...
export default function ErrorCard({ className, message, variant, asElement = "div" }: TProps) {
  ...
  return (
    ...
    >
      ...
      {message && (
        <p className="text-muted-foreground bg-destructive/1-10 mt-1 w-full flex-1 rounded-lg px-3 py-2 text-left font-mono text-xs">
          {message}
        </p>
      )}
    </Element>

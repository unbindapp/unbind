...
func Uninstall(uninstallScriptPath string, logChan chan<- string) error {
	...
	err = runCommand(logChan, "kubectl", "create", "-f", "https://raw.githubusercontent.com/longhorn/longhorn/v1.12.0/uninstall/uninstall.yaml")
	if err != nil {
	...
	for {
		select {
		case <-timeout:
			nbSend(logChan, "Warning: Longhorn uninstall job timed out, continuing anyway")
			goto cleanup
		case <-tick:
			...
			if err == nil && strings.TrimSpace(string(output)) == "True" {
				nbSend(logChan, "Longhorn uninstall job completed successfully")
				goto cleanup
			}
		}
	}
cleanup:
	...
	for _, path := range longhornPaths {
		...
		if err != nil {
			nbSend(logChan, fmt.Sprintf("Warning: Failed to remove %s, continuing anyway", path))
		}
	}

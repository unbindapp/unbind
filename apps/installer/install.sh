...
LATEST_VERSION=$(curl -s https://api.github.com/repos/unbindapp/unbind/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
...
curl -L "https://github.com/unbindapp/unbind/releases/download/$LATEST_VERSION/unbind-installer-$ARCH.gz" -o "installer.gz"
curl -L "https://github.com/unbindapp/unbind/releases/download/$LATEST_VERSION/unbind-installer-$ARCH.gz.sha256" -o "expected.sha256"

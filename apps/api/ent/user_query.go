...
func (_q *UserQuery) QueryOauth2Tokens() *Oauth2TokenQuery {
	query := (&Oauth2TokenClient{config: _q.config}).Query()
...
func (_q *UserQuery) QueryCreatedBy() *GithubAppQuery {
...
func (_q *UserQuery) WithOauth2Tokens(opts ...func(*Oauth2TokenQuery)) *UserQuery {
	query := (&Oauth2TokenClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOauth2Tokens = query
	return _q
}
...
func (_q *UserQuery) WithCreatedBy(opts ...func(*GithubAppQuery)) *UserQuery {
	query := (&GithubAppClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withOauth2Tokens; query != nil {
		if err := _q.loadOauth2Tokens(ctx, query, nodes,
	...
	if query := _q.withCreatedBy; query != nil {
		if err := _q.loadCreatedBy(ctx, query, nodes,
...
func (_q *UserQuery) loadOauth2Tokens(ctx context.Context, query *Oauth2TokenQuery, nodes []*User, init func(*User), assign func(*User, *Oauth2Token)) error {
...
func (_q *UserQuery) loadCreatedBy(ctx context.Context, query *GithubAppQuery, nodes []*User, init func(*User), assign func(*User, *GithubApp)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)

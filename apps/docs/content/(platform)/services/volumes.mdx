...
Open the volume and press **Detach** next to the service under **Connection**, then press **Deploy** on the changes bar. The service restarts without the volume, and the data stays in the volume.

The volume shows as detaching until the old version of the service stops. After that it waits in the environment, unattached. Open it and pick a service under **Mount to Service** to attach it again, to the same service or another one.

A database volume cannot be detached. The database needs it to run.
...
- **A volume can be deleted only when nothing is attached.** [Detach](#detach) it first, then open it and use **Delete Volume**. This cannot be undone.

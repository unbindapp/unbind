...
The MCP server has no logic of its own. Every tool is an operation of the HTTP API, called the same way, with the same validation and the same permissions. If the [reference](/api/reference/teams/list-teams) says an operation does something, the tool with that name does exactly that.
...
| Changes                       | `apply-changes`, with a dry run that shows what would be affected                  |
| You                           | `me`                                                                               |
...
- Agents call `me` first to learn their role and limits.

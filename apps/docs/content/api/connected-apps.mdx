...
- **Capabilities**: read variable values, read logs and read webhook URLs. All off by default. See [Capabilities](/api/authentication#capabilities).
...
Your avatar > **Account Settings** > **Connected Apps** lists every app with its role, its resources, its capabilities and when it was last used. **Revoke** cuts it off right away. The app has to ask again to get back in.

...
>(
  (
    ...
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex px-3 font-bold gap-1 relative group/trigger before:w-full before:h-full before:min-w-[48px] before:min-h-[48px] before:left-1/2 before:top-1/2 before:-translate-x-1/2 before:-translate-y-1/2 before:z-[-1] select-none z-0 before:bg-transparent before:absolute touch-manipulation items-center justify-between whitespace-nowrap rounded-lg not-touch:hover:bg-border active:bg-border border bg-transparent leading-tight py-1.5 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus-visible:ring-1 focus-visible:ring-offset-2 focus-visible:ring-primary/50 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
        className
      )}
      {...props}
    >
    ...
    return (
      <SelectPrimitive.Portal>
        <SelectPrimitive.Content
          ...
          ref={ref}
          className={cn(
            "max-w-[var(--radix-popper-available-width)] max-h-[min(20rem,var(--radix-popper-available-height))] relative shadow-lg shadow-shadow/[var(--opacity-shadow)] z-50 overflow-hidden rounded-lg border bg-popover text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
            position === "popper" &&
